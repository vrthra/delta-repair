...
Data cleansing and data repair are time-consuming tasks for developers, especially in fields that aggregate data from multiple resources (e.g., data warehousing). Often times,
...
We evaluate \brepair on 806 (real-world) invalid inputs using four well-known input formats, namely INI, TinyC, SExp, and cJSON. In our evaluation, we found that \approach is highly effective and efficient in input repair. It repairs 77\% of invalid inputs and recovers 91\% of input data, within four minutes. It is up to 35\% more effective than \ddmax, the previously best known approach.
...
insights~\cite{scaffidi2008accommodating}. %\todo{add concrete example use case} 
For instance, \textit{data warehouses} collect and aggregate data from several %trusted and untrusted 
sources. 
Unfortunately, data sources may disagree on the format specification, %e.g.,
%\recheck{
consequently leading to different %variances in the
implementations of the specification. %}.
For example, (1)~JSON libraries implement slightly different
definitions of the JSON formats~\cite{harrand2021behavioral,seriot2016parsing};
(2)~different database systems support %, each supporting a
slightly different SQL formats~\cite{arvin2018comparison}; (3)~there are a variety of markdown
...
are created by hand, and the process of manual generation can be
error-prone~\cite{scaffidi2008topes,mucslu2015preventing}.
...
More importantly, \citet{kirschner2020debugging} and \citet{scaffidi2008topes} demonstrate the prevalence of invalid inputs and the importance of input repair in practice. % without grammars or program analysis.
%using \ddmax. %}.
...
rather than \emph{inserted.} Hence, by handling only spurious insertions, \ddmax misses
...
and its descendants address this concern for reducing failure-inducing inputs, however, this problem remains open for input repair. Notably, a similar constraint also applies to \ddmax, where
the unstated assumption is that each non-failure-inducing fragment
can be added to the passing subset without inducing a failure. When this
assumption is not met (as in the case of inputs with a rich structure such as
conforming to a grammar) the repairs produced can be suboptimal as the
...
\recheck{\brepair denotes ``\underline{\textsc{F}}eedback-driven nput \underline{\textsc{SYNTH}}esis''}}
...
%\recheck{
To the best of our knowledge, \approach is the \textit{first} approach to repair invalid inputs without an input specification (e.g., grammar) or program analysis: It repairs invalid inputs %achieves this 
...
Results show that %given input processors that can accurately
%signal \emph{incompleteness} or \emph{incorrectness} and % when given
%invalid inputs,
\brepair is highly effective (77\% repairs within four minutes), and it is up to 35\% more effective than \ddmax.
...
\brepair technique can repair inputs with rich structure, including multiple faults.
...
We conclude this paper with the discussion of related work and conclusions in \Cref{sec:related_work} and \Cref{sec:conclusion}, respectively.
...
All experiments were conducted within a timeout of four minutes per repair technique per input. % file.
...
lexical \ddmax, syntactic \ddmax and \approach. \Cref{tab:data-recovery-and-loss} %and \recheck{Figure X
highlights the amount of data %loss and data
recovered and lost % achieved 
by each approach. %, as average over each file. %Our evaluation r
...
\noindent
\textbf{Repair via input Synthesis:} The input synthesis repair operation %(i.e., the insertion operation) 
of \approach poses the risk of introducing input elements with unintended semantic consequences. 
%Such operations % into the input, which 
Specification, insertion operations may lead %cause further 
data corruption and information distortion. To mitigate this threat, \approach provides several candidate repairs ranked by the edit distance 
of the each repair candidate from the original input.
%to the original input. 
Furthermore, we encourage end-users to employ %\approach  %recommend to 
the results of %provided by 
\approach % candidates 
%as potential repairs that allows 
as potential repair candidates, out of which to select the best semantically fit repair for their use.
...
%\recheck{
%In this paper, we 
This paper presents \approach, an input repair approach that employs input synthesis and lightweight failure feedback (i.e., incomplete and (in)correct checks) to repair invalid inputs. Our approach is black-box, does not require program analysis or an input grammar. We evaluate the effectiveness, data recovery and efficiency of our approach, in comparison to the state-of-the-art methods. We show that \approach is highly effective and efficient in input repair---it completes the repair of four in five invalid inputs (77\%), within a four minute timeout. Furthermore, we demonstrate that our approach is up to 35\% more effective than the best 
%In comparison to the 
best baseline---(syntactic) \ddmax, without using an input grammar. Overall, this work demonstrates that lightweight failure feedback and input synthesis are important for the effective repair of invalid inputs, especially in the absence of an input specification (e.g., grammar) and white-box program access. In the future, we plan to investigate how to improve the performance of our approach by learning input semantics and constraints, or via program analysis and the knowledge of the input specification. %grammar.
%}
%  more effective thja
