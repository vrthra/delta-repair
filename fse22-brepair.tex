...
\<repair()>. This function does a binary search on the argument string looking
for the parse boundary where the return value chages from \emph{incomplete} to
\emph{incorrect}. The \<boundary> value is then used to start the first repair
thread, which starts at edit distance \<0>. The edit distance spacifies the
amount of corrections that were applied on a thread.
...
repairs on it which are defined in \Cref{lst:repair}.
\begin{lstlisting}[caption=\brepair repairs,label={lst:repair}]
def apply_delete(item):
    return Repair(item.inputstr[:item.boundary] +
