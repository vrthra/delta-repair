...
def repair(inp):
    ...
    thread = {0: [Repair(inp, boundary)]}
    return find_fixes(thread)

def find_fixes(thread):
    edit_dist = 0
    while True:
        cur_items = thread[edit_dist]
        comp = []
        for item in sample_items_by_mask(cur_items):
            for i in item.repair_and_extend():
                thread[edit_dist + 1].append(i)
...
\begin{lstlisting}
class Repair:
    slots: inputstr, boundary:
    def apply_delete(self):
        return Repair(self.inputstr[:self.boundary] +
                      self.inputstr[self.boundary + 1:],
		      self.boundary)

    def apply_insert(self):
        new_items = []
        for i in string.printable:
            v = self.inputstr[:self.boundary] +
	    i + self.inputstr[self.boundary:]
    ...
    def apply_modify(self):
        ...
        for i in string.printable:
            v = self.inputstr[:self.boundary] +
	    i + self.inputstr[self.boundary + 1:]
            new_items.append(Repair(v, self.boundary))
        return new_items
\end{lstlisting}
...
class Repair:
    def extend_item(self):
        nv = 0
        while True:
            if (self.boundary + nv) > len(self.inputstr):
                self.boundary = self.boundary + nv - 1
                return self
            s = Repair(self.inputstr, self.boundary + nv)
            if s.is_incomplete():
                nv += 1
                continue
            if s.is_incorrect():
                ...
                return self
            if s.is_complete():
                self.boundary = self.boundary + nv
                return self
