...
Notably, two major \ddmax limitations include its (1) \textit{limited repair operation} (only deletion), (2) inability to completely repair \textit{rich input structures }(e.g., invalid inputs containing multiple faults) due to inherent assumptions. % (e.g., its search span)..
...
\ddmax is its inability to handle \textit{inputs with rich structure}. We show that our
