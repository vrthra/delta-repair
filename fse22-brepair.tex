...
\begin{tabular}{|l | c | r  r | r  r |}
\hline
& \textbf{\#Repaired}&  \multicolumn{2}{c|}{\textbf{Runtime (s)}} & \multicolumn{2}{c|}{\textbf{\#Prog. Runs}}  \\
...
Let us evaluate the time performance of our approach, i.e., we investigate how long it takes \approach to repair an invalid input, in comparison to the state-of-the-art ddmax. %In addition, w
We also evaluate the number of repair attempts, i.e., number of program runs %test experiments with input fragments, 
performed by each approach. \recheck{\autoref{tab:efficiency} and figure X} illustrate the efficiency of our approach compared to the state-of-the-art methods. 

In this evaluation, we found that \textit{\approach is efficient in input repair: It is reasonably fast, it takes about \recheck{2.8} seconds to repair an invalid input file, on average}. 
\approach is more efficient than lexical ddmax (\recheck{5} seconds), but less efficient than synatactic ddmax (\recheck{1.2} seconds). 
%The time performance of 
This implies that \approach is twice as fast as %er than 
lexical ddmax, but twice slower than syntactic ddmax. This is expected since the execution time of %repair attempts (i.e., 
program runs %) 
is the main time-consuming component of \approach, as well as ddmax. However, this cost is much lower for syntactic ddmax (\recheck{X} runs) 
...
since it leverages the input grammar to reduce the number of test experiments. 
%, i.e. test experiments with input fragments. 
\recheck{\autoref{tab:efficiency}  and Table/Figure X show} the %Inspecting 
the number of %repair attempts (aka 
program run performed for all approaches. %) 
...
%We observe that 
These results suggest that the number of program runs is the main performance bottleneck of all three approaches. % to the 
