...
% Consider "Repairing Inputs with Input Synthesis" -- AZ
...
In this paper, we propose \brepair\footnote{
...
\recheck{To the best of our knowledge, \approach is the \textit{first} approach to synergistically combine\textit{ lightweight failure feedback and input synthesis to repair invalid inputs}}. We evaluate \brepair using \recheck{806} (real-world) invalid inputs belonging to four well-known input formats (e.g., cJSON). % and TinyC). 
...
%invalid inputs,
\brepair is highly effective (77\% repairs within four minutes), and it is up to \recheck{35\%} more effective than \ddmax.
...
\begin{tikzpicture}[node distance=\nodedst]
    ...
    \node[blockdiagramblock,below=of search] (pop) {Apply Repairs};
    \draw[blockdiagramarrow] (search) -- (pop); %node[simpletext,pos=.5,right,font=\scriptsize] {}; % PushPQ
    \node[blockdiagramblock,below=of pop] (correct) {\mbox{Valid Input?}};
    \draw[blockdiagramarrow] (pop) -- (correct);
    \node[simpletext,below=.3 of correct] (return) {\mbox{Return input}};
    \node[blockdiagramblock,right=of correct] (tryDelete) {\mbox{Extend Threads}};
    \node[blockdiagramblock,above=of tryDelete] (tryNewInsert) {\mbox{Select Threads}};
    ...
    \pic[] at ([yshift=-3mm]correct.south east) {numbering=5};
    \pic[] at (tryDelete.south east) {numbering=2};
    \pic[] at (pop.south east) {numbering=2};
    \pic[] at (tryNewInsert.south east) {numbering=3};
    %TODO
\end{tikzpicture}
% \vspace*{-0.1in}
