...
def apply_insert(item):
    ...
    for i in string.printable:
        ...
        new_items.append((v, item.boundary))
    return new_items
...
def extend_item(item):
    ...
    while True:
        if (item.boundary + nv) > len(item.inputstr):
            return (item.inputstr, boundary + nv - 1)
        s = (item.inputstr, item.boundary + nv)
        if is_incomplete(s):
            nv += 1
            continue
        if is_incorrect(s):
            return (item.inputstr, boundary + nv - 1)
        if is_complete(s):
            return (item.inputstr, boundary + nv)
\end{lstlisting}
