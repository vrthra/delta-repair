...
\begin{abstract}
Data cleansing and data repair are time-intensive tasks for developers, especially in fields that aggregate data from multiple resources (e.g., \recheck{x}). Often times,
...
it follows a given format, and hence processable by the conforming program.
...
semantics of the input data. We propose an approach called \brepair that leverages lightweight error-feedback and input synthesis to repair invalid inputs.\todo{We need to clarify the feedback requirement, correctness and incompleteness}
...
We evaluate \brepair on \recheck{806} (real-world) invalid inputs using four well-known input formats, namely INI, TinyC, SExp, and cJSON. In our evaluation, we found that it is highly effective and efficient in input repair. It repairs \recheck{81}\% of invalid inputs and recovers \recheck{88\%} of input data, within two minutes. It is up to \recheck{18\%} more effective than \ddmax -- the previously best known approach.
...
insights. Unfortunately, data sources may disagree on the format specification, %e.g.,
\recheck{consequently leading to different %variances in the
...
\Cref{tab:ddmaxlimitations} and \autoref{sec:rich-input-structures} illustrate %some of
...
%\todo{discuss the repair to subset vs. insertion, refer to \autoref{sec:input-synthesis}}.
...
%We propose \brepair which does not have these limitations. \todo{refer to \autoref{sec:brepair}}
 %	\item \textbf{}	
...
\emph{inserting} a character at any index, or \emph{modifying} a character at
any index. Note that in the below listings, \<Repair> is an tuple that allows
...
def apply_delete(item):
    ...
    return Repair(inp, item.boundary)

def apply_insert(item):
    new_items = []
    for i in string.printable:
        v = item.inputstr[:item.boundary] +
            i + item.inputstr[item.boundary:]
        new_items.append(Repair(v, item.boundary))
    return new_items

def apply_modify(item):
    new_items = []
    for i in string.printable:
        v = item.inputstr[:item.boundary] +
            i + item.inputstr[item.boundary + 1:]
        new_items.append(Repair(v, item.boundary))
    return new_items
\end{lstlisting}
...
that, it tries to ensure that no non-redundant repairs are discarded. For
example, given a JSON fragment \<[1,2> a repair of \<[1,23>, \<[1,24>
and \<[1,25> are redunant but \<[1,2,> is not because the last repair
represents a change in semantics for the parser.
...
def extend_item(item):
    nv = 0
    while True:
        if (item.boundary + nv) > len(item.inputstr):
            return Repair(item.inputstr, boundary + nv - 1)
        s = Repair(item.inputstr, item.boundary + nv)
        if is_incomplete(s):
            nv += 1
            continue
        if is_incorrect(s):
            return Repair(item.inputstr, boundary + nv - 1)
        if is_complete(s):
            return Repair(item.inputstr, boundary + nv)
\end{lstlisting}
...
def repair_and_extend(item):
    e_arr = [extend_item(apply_delete(item))]
    for i in apply_insert(item) + apply_modify(item):
...
\item \emph{Insertion.} At the same point, \brepair also tries to modify a single character and
checks whether the modification changes the valid substring.
\item Single character deletion, insertion as well as modification is an edit distance of one.
...
thread that already contains one character appended, or modify a character in
place.
...
\noindent
\textbf{\textit{Data Recovery:}} \autoref{tab:data-recovery} %and \recheck{Figure X
