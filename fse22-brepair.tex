...
\dpass & \text{\hphantom{else }if $|\cfail - \Delta_i| = 1$ (``base case$^{\textcolor{red}{a}}$'')} \\
%
...
%
  \ddmax_2\bigl(\dpass, \min(|\cfail - \dpass|, 2n)\bigr) & \text{else if $n < |\cfail - \dpass|$ (``increase granularity$^{\text{\textcolor{red}{b}}}$'')} \\
...
\textcolor{red}{a}: Bugfix: This base case is necessary to ensure that \letterboxed{1*1} does not violate the invariant $n \leq |\Delta|$.\\
\textcolor{red}{b}: Bugfix: We should look for minimum of the remaining string so that invariant $n \leq |\Delta|$ is not violated for \letterboxed{\{*"":2\}}.
\end{minipage}
\end{boxedminipage}
