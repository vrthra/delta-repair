...
In this work, we show that incorporating light-weight error-feedback (e.g., input incompleteness) t\todo{what is lightweight error-feedback?} to parsers
...
However, \recheck{formats such as markdown~\cite{gruber2004markdown} does not have
a formal grammar}\todo{how do you mean? this is not true, there are markdown grammars, ANTLR also has one, Nikolas used some in his Tribble eval too.}; Documents in languages such as \<C> can contain semantic
...
\begin{itemize}
	\item \textbf{Repairing Rich Input Structures:} \todo{add bug fixes here too . reference to \autoref{sec:rich-input-structures}.} \recheck{We identify one of the major limitations to \ddmax -- the limitation due to rich structure, which can be fixed only by requiring additional constraints, and We identify one of the major limitations to \ddmax -- the limitation due to rich structure, which can be fixed only by requiring additional constraints.}
	\item \textbf{Input Synthesis:} \todo{discuss the repair to subset vs. insertion, refer to \autoref{sec:input-synthesis}}. 
	\item \textbf{Lightweight Failure Feedback:} We propose \brepair which does not have these limitations. \todo{refer to \autoref{sec:brepair}}
%	\item \textbf{}	
