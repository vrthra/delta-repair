...
Note that in the below listings, \<Repair> is a tuple that allows
...
def insert_char(item, i):
    ...
    if LAST_INSERT_ONLY:
        ...
        new_item = Repair(v, item.boundary, mask= \ 
            '%s_I%d' % (item.mask, item.boundary))
...
The sampling procedure attempts to discard redundant repairs so that the number of
simultanious threads we have to maintain does not grow unbounded. While doing
that, it %tries to 
ensures that no unique repairs are discarded. For
...
\end{lstlisting}
The output from \<find\_fixes()> (\Cref{lst:findfixes}) is a set of repairs with
...
count of a particular edit distance to a configurable \<N> value\todo{what is the value of N used in the eval. and how was this determined?}. If the number of
...
This section describes the experimental setup of this work. In particular, we discuss %specify %especially 
...
In this work, we compare the performance of \approach to the following state-of-the-art techniques: %. Specifically, we compare to the following:
...
\item[c.] \textbf{\ddmax:} Kirschner et al.~\cite{kirschner2020debugging} proposed %This refers to the 
...
\subsubsection*{\bf Subject Programs:} \Cref{tab:subject-programs} provides the details of the subject programs employed in our experiments. We used four programs belonging to four input formats, namely INI, cJSON, SExpParser and TinyC in our experiments. All of these programs are written in hundreds of (375 to 3075) LOC of C programming language, and they are between 6 to 21 years old.
...
%the experiments 
on a Dell Precision 7510 with four physical CPU cores %(8~virtual cores)
