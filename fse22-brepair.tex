...
\begin{abstract}
Data cleansing and data repair are some of the most intensive tasks in many
fields that aggregate data from multiple resources. In many cases, the data may
be entered by hand, and while ostensibly conforming to a given format, is often
unparsable by machines. In such cases, a
data engineer has to manually repair the corrupt data such that it follows a
given format, and hence processable by the next program. Such manual repair can
...
insights. Unfortunately, not every source may agree on the format. For example,
there are numerous JSON libraries with slightly different definitions of what
the JSON format is~\cite{harrand2021behavioral,seriot2016parsing}. Further,
there are numerous data base systems with slightly different supported SQL
formats~\cite{arvin2018comparison}, and a variety of markdown syntaxes with
subtle differences~\cite{visnoviz2019comparison}. The subtle difference between
various \<C> dialects are also well known.
That is, it is often
impossible to parse a document that was generated with a different
implementation in mind. Another issue is that many such documents are created
by hand, and the process of manual generation can be error-prone.

The problem is that such documents can be \emph{almost} but \emph{not quite}
parsable by the parser at hand. So, consuming such resources, and aggregating
them often involves trying to maximally parse documents by minimizing
parse errors. If the parser at hand conforms to a context-free grammar, and the
formal grammar is available, this issue can be solved using
\emph{error-correcting} parsers~\cite{aho1972minimum}.
...
place. However, formats such as markdown~\cite{gruber2004markdown} does not have
...
\ddmax works very well when the input document and parser follows the
constraints it imposes. Notabily, the independence of deltas (that each delta
can be added or removed independently of others), and one is looking for a
maximal \emph{subset} of deltas.

Unfortunately, these two constraints form the biggest limitations of \ddmax.
...
\caption{Invalid JSON which \ddmax fails to repair}
\label{fig:bad-json-input-asterisks}
\end{figure}

\noindent\textbf{Limited repairability.} One of the biggest limitations of
\ddmax is its assumption that the input can be decomposed into independent
fragments. \ddmax works very well when the input can be split into independent
deltas. The problem is that in most cases, the input may have a rich structure,
and may not be decomposable effectively into independent deltas. As an example,
consider \Cref{fig:bad-json-input-asterisks} similar to Kirchner et
al.~\cite[Figure 1]{kirschner2020debugging}. Here, the JSON string is invalid
because it contains three \<*> characters. The operation of \ddmax is as follows:

\begin{enumerate}
\item Split the string in half, resulting in\\
$\Delta_1$=\letterboxed{\{\ "item":\ "Apple",} \\% 18
$\Delta_2$=\letterboxed{\ "price":\ ***3.45\ \}}. % 19
\item Neither of these result in valid independent fragments. That is, removing
neither results in the rest of the string becoming valid. Hence split it
again into quarters, we focus on ($\Delta_{2}$) since it contains the invalid fragment,
and hence if removed, makes the rest of the string valid:\\
$\Delta_{2.1}$ = \letterboxed{\ "price":} \\ %9
$\Delta_{2.1}$ = \letterboxed{\ ***3.45\ \}}%10.
\item None of these fragments can be excluded independently resulting in a
no-failure input. So we try again. This time, let us focus in $\Delta_{2.2}$ since it
contains the invalid input. We split that into:\\
$\Delta_{2.2.1}$ = \letterboxed{\ ***3\\}\\ %5
$\Delta_{2.2.2}$ = \letterboxed{.45\ \}}. %4
\item Excluding neither will produce a valid JSON input. So we try again,
focusing on $\Delta_{2.2.1}$ now since it contains the failuring inducing fragment;
producing:\\
$\Delta_{2.2.1.1}$ = \letterboxed{\ *}\\ %2
$\Delta_{2.2.1.2}$ = \letterboxed{**3}\\ %3
neither will succeed in removing the invalid fragment completely.
\end{enumerate}
That is, this particular invalid JSON string can't be repaired by \ddmax. This
happens because the constraint, that each frament be independently removable is
not applicable to JSON.

\noindent\textbf{Limited Options for Repair.} The second major limitation of
...
\begin{cases}
  ...
  \textcolor{red}{\dpass} & \text{\textcolor{red}{\hphantom{else }if $|\cfail - \Delta_i| = 1$} (``base case$^{\textcolor{red}{a}}$'')} \\
%
...
\begin{tikzpicture}[node distance=\nodedst]
    ...
    \node[blockdiagramblock,right=3 of search.north east] (pop) {Pop element from PQ};
    \draw[blockdiagramarrow] ([yshift=-\arrowsep]search.north east) -- ([yshift=-\arrowsep]pop.west |- search.north east) node[simpletext,pos=.5,above,font=\scriptsize] {Push prefix to PQ};
    \node[blockdiagramblock,right=of pop] (correct) {Input is correct?};
    \draw[blockdiagramarrow] (pop) -- (correct);
    \node[simpletext,right=of correct] (return) {Return input};
    \node[blockdiagramblock,below=1 of correct] (tryOrigInsert) {Insert from orig. input};
    \node[blockdiagramblock,left=of tryOrigInsert] (tryDelete) {Delete from orig. input};
    \node[blockdiagramblock,left=of tryDelete] (tryNewInsert) {Insert Character};
    \draw[blockdiagramarrow] (correct) -- (return) node[simpletext,pos=.4,above] {\pass};
    \draw[blockdiagramarrow] (correct) -- (tryOrigInsert) node[simpletext,pos=.4,right] {\fail};
    \draw[blockdiagramarrow] ([xshift=-2 * \arrowsep]tryOrigInsert.south) to[out=270,in=315,looseness=0.6] node[simpletext,pos=.5,below,sloped,inner sep=1pt] {Incomplete?} ($(tryNewInsert.south west) - (.7,.3)$) to[out=135,in=270] (search);
    \draw[blockdiagramarrow] (tryOrigInsert) -- (tryDelete);
    \draw[blockdiagramarrow] (tryDelete) -- (tryNewInsert);
    \node[simpletext,below=.3 of tryDelete.south,inner sep=1pt] (pqins1) {push PQ};
    \draw[blockdiagramarrow,dashed] (tryDelete) -- (pqins1);
    \node[simpletext,above=.5 of tryNewInsert,inner sep=1pt] (pqins2) {push PQ};
    \draw[blockdiagramarrow,dashed] (tryNewInsert) -- (pqins2) node[simpletext,left,pos=.5] {Incomplete?};
    \draw[blockdiagramarrow] ([xshift=2 * \arrowsep]tryNewInsert.north) to[out=90,in=270] (pop);
    %TODO
\end{tikzpicture}
% \vspace*{-0.1in}
    \caption{Workflow of \brepair}\label{fig:ddmax_flowchart}
    % \vspace*{-0.1in}
\end{figure*}
...
\begin{table}[!tbp]\centering
\caption{Efficiency of \approach vs. the state-of-the-art (ddmax), lower runtime, smaller number of program runs and significant \% improvement in efficiency are in \textbf{bold}}
%\vspace{-0.5cm} 
\begin{tabular}{|l | c | r  r | r  r |}
\hline
& \textbf{\#Repairs}&  \multicolumn{2}{c|}{\textbf{Runtime (ms)}} & \multicolumn{2}{c|}{\textbf{\#Program Runs}}  \\
\textbf{Techniques} & \textbf{Inputs} &  \textbf{Avg.}  & \textbf{Total} & \textbf{Avg.}  & \textbf{Total} \\
\hline
\textbf{Lex. ddmax} & & & & & \\
\textbf{Syn. ddmax} & & & & & \\
\textbf{\approach} & & & & & \\
...
Let us evaluate the time performance of our approach, i.e., we investigate how long does it take \approach to repair an invalid input, in comparison to the state-of-the-art ddmax. In addition, we also evaluate the number of repair attempts, i.e., test experiments with input fragments, performed by each approach. \recheck{\autoref{tab:efficiency} and figure X} illustrate the efficiency of our approach compared to the state-of-the-art methods. 

In this evaluation, we found that \approach is efficient in input repair. It is reasonably fast, it takes about \recheck{three} seconds to repair an invalid input file on average. 
\approach is more efficient than lexical ddmax, but less efficienct than synatactic ddmax. 
The time performance is twice faster than lexical ddmax, and two times slower than syntactic ddmax. This is expected since the main time consuming component of these approaches is the number of repair attempts and both lexical ddmax and \approach perform more test experiments than syntactic ddmax, since it levergaes the input grammr to reduce the number of test experiments. 
%, i.e. test experiments with input fragments. 
Inspecting the number of repair attempts (aka program runs) shows that \approach performed \recheck{X} as less repair attempts as lexical ddmax, and 
\recheck{X} as many repair attempts as lexical ddmax. 
We observe that the number of repair attempts (program runs) is the main performance bottlenec of all three approaches. % to the 
...
\begin{result}
Our approach is reasonably fast, 
it repairs an invalid input within \recheck{three} seconds, on average. \approach 
is twice as effective as lexical ddmax, but only half as fast syntactic ddmax. 
\end{result}
