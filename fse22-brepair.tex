...
In this work, we show that incorporating light-weight error-feedback\todo{what is lightweight error-feedback?} to parsers
is sufficient to repair any corrupt input data with maximal closeness to the
semantics of the input data. We propose an approach called \brepair that leverages error-feedback (e.g., data incorrectness and incompleteness) to repair invalid inputs.\todo{We need to clarify the feedback requirement, correctness and incompleteness}
...
We evaluate \brepair on \recheck{806} real-world and mutated invalid inputs using four well-known input formats, %(and their parsers), 
namely INI, %(\recheck{X}), 
TinyC, % (\recheck{X}), 
SExp, % (\recheck{X}) 
and cJSON. % (\recheck{X}). 
In our evaluation of \brepair, we found that it is highly effective and efficient in input repair. It repairs 81\% of invalid inputs within two minutes, and outperforms \ddmax %in input repair 
by up to \recheck{18\%}. 
\end{abstract}
...
In this paper, we provide a solution.
...
bFuzzer is getting submitted at FSE too.}.
