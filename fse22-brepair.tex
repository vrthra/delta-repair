...
be entered by hand, and is often almost parsable by machines. In such cases, a
...
In this work, we show how to incorporate error-feedback to parsers such that
the feedback is sufficient to repair the input string with maximal closeness to
the semantics of input data.

Given nothing but a conforming program, and a corrupt input, our \approach
provides a set of repairs prioritized by the distance of the repair from the
original input.

We show that our approach can remove the limitations inherent in ddmax, the
previously best known approach. In our evaluation of \approach, we found that
it can outperform ddmax in input repair, and can repair all input corruptions
that were induced.
\end{abstract}
...
\noindent
\textbf{RQ2 Data Recovery and Data Loss:} In this experiment, we investigate the amount of data recovered (and lost) by our approach (\approach).  We also examine the contribution of the insertion  of input elements (i.e., input synthesis) to the effectiveness of our approach. To this end, we compute the file sizes of repaired inputs, and the edit distance between the repaired input and the original input using Levenshtein distance. In addition, we compare the data loss and recovery achieved by our approach to that of the state of the art, i.e. ddmax. For this experiment, we computed the (difference in) files sizes and the Levenshtein distance between the original input and the repaired inputs. For a fair evaluation of data recovery, we used 475 inputs that were completely repaired by all three approaches (lexical ddmax, syntactic ddmax and \approach) before a timeout.  For a balanced and tractable evaluation of data loss, we employed the set of 321 completely repaired inputs by all three approaches which %(321) inputs that 
the Levenshtein distance algorithm could compute their edit distance within a threshold of 750 edit distances (about 30 seconds). % and which were completely repaired by all three approaches before a timeout. 
%This resulted in a set of set of (321) inputs for the evaluation of data loss. 
\recheck{Table X and Figure X} illustrate the data loss and recovery achieved by each approach. 
