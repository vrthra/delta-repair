...
an important repair requirement. Consider the sample invalid input in \textit{row one} of \Cref{tab:ddmaxlimitations} (i.e., \letterboxed{\{\ "name":\ "Dave"\ "age":\ 42\ \}}) which is invalid because of a \textit{missing comma separator} between the two values in the JSON object. Due to the limited repair options of \ddmax (i.e., only deletion), its resulting repair (i.e., \letterboxed{\ \ \ \ 42\ }) is incomplete and leads to a huge (75\%) data loss (21 out of 28 bytes). %, % $\approx$ . 
%  data loss).
...
examples show.
%\recheck{
As an example, consider the invalid input in \textit{row three} of \Cref{tab:ddmaxlimitations} (i.e.,  \letterboxed{\{"ABCD":[*"1,2,3,4,5,6"]*\}}). Due to the multiple faults in this input, the resulting \ddmax repair (i.e., \letterboxed{123456}) is \textit{again} \textit{incomplete} and leads to a huge (77\%) data loss.
%&
% &
%\letterboxed{\{"ABCD":["1,2,3,4,5,6"]\}} &
%
...
resulting in an increase of maximal parse prefix. Out of these two insertion candidates, inserting comma results in the maximum advancement of the parse boundary, resulting in the
...
this information, obtaining this information by external instrumentation is
possible as demonstrated by Bj\"orn et al.~\cite{mathis2019parser}. If
the program is hard to instrument, adding failure feedback that we require is trivial. 
%not difficult. 
}
%In our evaluation, u
...
%invalid inputs, 
\brepair is \recheck{X\%} more effective and \recheck{X\%} \recheck{efficient} %can provide repairs that are more maximal 
than \ddmax. 
%, and correctly
%pass the parser.
...
The rest of this paper is structured as follows: \Cref{sec:rich-input-structures} highlights the limitations of the state-of-the-art input repair method (\ddmax) and illustrates how \brepair overcomes these limitations. In \Cref{sec:input-synthesis} we discuss how \brepair performs input synthesis, and \Cref{sec:brepair}  describes the \brepair algorithm. We describe our experimental setup and findings in \Cref{sec:experimental-setup} and \Cref{sec:results}. We address the limitations and threats to the validity of this work in \Cref{sec:threats}
...
A major limitation of \ddmax is that it is ineffective when the input needs to
conform to specifications such as context-free grammars. We discuss two
...
contains multiple errors. While in this eaxmple, the data loss that occurred
...
removed, results in the solution \letterboxed{123456}, which clearly has
suffered significant data loss including the loss of structure and change in
input fragment type from string to number.
Hence, \ddmax can't repair inputs with multiple faults effectively.
...
algorithm of \ddmax.
As an example, consider a variant of the previos input: \letterboxed{[*+]}.
...
semantics. This particular assumption need not hold when we are dealing with
...
Of course, if  the input processor conforms to this constraint, \ddmax will
...
\begin{enumerate}
\item \brepair starts by executing a binary search for the boudary where the
...
next index is appeneded, resulting in
\letterboxed{\{\"ABCD":["}. %1,2,3,4,5,6"]*\}}.
...
\end{enumerate}
This completes the repair of the given input.

That is, \brepair has no problem with rich inputs containing multiple errors.
...
\item At this point, \brepair applies \emph{deletion}, \emph{insertin}, or
...
\section{\approach}
\label{sec:brepair}
The main strength of \brepair is its repertoire of repairs (\Cref{lst:repairs}).
The input string can be modified by \emph{deleting} any character, or
\emph{inserting} a character at any index.
Note that in the below listings, \<Repair> is an tuple that allows
...
def insert_char(item, i):
    ...
    return_lst = []
    if LAST_INSERT_ONLY:
        v = item.inputstr[:item.boundary] + i + suffix
        new_item = Repair(v, item.boundary, mask='%s_I%d' % (item.mask, item.boundary))
        ...
        for k in range(item.boundary):
            v = item.inputstr[:k] + i + item.inputstr[k:item.boundary] + suffix
...
from \emph{incomplete} to \emph{incorrect}.
\begin{lstlisting}[caption=binary search,label={lst:bsearch}]
def binary_search(array, left = 0, right = None):
    left, right = 0, len(array) - 1
    if is_incomplete(Repair(array, right)):
        return len(array)-1
    while left + 1 < right:
        middle = (left + right) // 2
        if is_incomplete(Repair(array, middle)):
            left = middle
        else:
            right = middle
    return left
\end{lstlisting}
The \<boundary> value is then used to start the first repair
thread, which starts at edit distance \<0>. The edit distance specifies the
amount of corrections that were applied on a thread. This thread is then passed
to \<find\_fixes()> (\Cref{lst:findfixes}).
\begin{lstlisting}[caption=find fixes,label={lst:findfixes}]
def find_fixes(threads):
    edit_dist = 0
    while True:
        cur_items = threads[edit_dist]
        comp = []
        for item in sample(cur_items):
            for i in repair_and_extend(item):
                threads[edit_dist + 1].append(i)
                if is_complete(i): comp.append(i)
        if comp: return comp
        edit_dist += 1
\end{lstlisting}
The function \<find\_fixes()> takes in a set of threads. It then extracts
threads corresponding to the least edit distance. If the number of such threads
exeed a user-configured value, a constant number of threads are sampled
(\Cref{lst:sampling}) for continuation.
Then, each thread is processed individually. On each thread, a set of repairs
(\Cref{lst:repairs}) are applied. If the repair succeeds,
...
The sampling procedure tries discard redundant repairs so that the number of
simultanious threads we have to maintain does not grow unbounded. While doing
that, it tries to ensure that no unique repairs are discarded. For
...
\<[1,2,"x"]>. Hence, we use the repairs conducted on a string, the
...
After each repair, the new string is checked (\Cref{lst:extenditem}) to see if
...
def bsearch_extend_item(item):
    bs = binary_search(item.inputstr, left=item.boundary-1)
...
def lsearch_extend_item(item, nxt=1):
    while True:
        ...
        if is_incorrect(s):
            ...
            return item
\end{lstlisting}
If this succeds, the parse boundary after the repair
would be larger than the old parse boundar. Hence, if the parse boundary has
increased (\Cref{lst:repairandextend}). If the boundary has increased, the
...
\end{lstlisting}
The output frm \<find\_fixes()> (\Cref{lst:findfixes}) is a set of repairs with
the least edit distance from the passed input string.
...
\item \emph{Deletion.} At the point of boundary (the first such character), we set the current
edit distance to zero. Then, \brepair applies
deletion, and checks whether the deletion changes the valid substring.
\item \emph{Insertion.} At the same point, \brepair also tries to insert a single character and
checks whether the insertion changes the valid substring.

\item Single character deletion, as well as insertion is an edit distance of one.

\item If the update results in a change of size of the valid
...
% first, followed by the size of the valid substring.
%When there is a tie between deletion and insertion, deletion gets
%the priority. When two insertions are available, one is chosen randomly.
...
advances the valid substring. Similarly, we append one more character to any
thread that already contains one character appended.

\item Given that there are 128 printable ASCII characters, each insertion can increase
...
We evaluate the effectiveness of our approach using 806 invalid input files belonging to four different input formats, namely INI, cJSON, SExp, and TinyC.  \Cref{tab:input-details} provides details of the %number of
...
the two maximizing variants of delta debugging, i.e., \textit{lexical ddmax} and \textit{syntactic ddmax}.   \Cref{tab:effectiveness} and \Cref{fig:effectiveness} highlight the (overall) effectiveness of each approach %, as well as their effectiveness
...
\Cref{tab:repair-complementarity} shows that for lexical ddmax, our approach \textit{solely} repaired 15x as many invalid files as lexical ddmax (90 versus six), while it repairs more than twice as many files as syntactic ddmax (90 versus 41).
% \textit{one in five (20\% of) invalid inputs could be repaired solely by \approach}. \Cref{tab:repair-complementarity} and \Cref{fig:repair-complementarity} highlight the complementarity of \approach to ddmax.
...
\noindent
\textbf{\textit{Data Recovery:}} \Cref{tab:data-recovery} %and \recheck{Figure X
highlight the data %loss and data
...
We found that \textit{\approach achieves a low data loss of about 13 edit distances,on avarage},  which is \textit{up to 23 times lower than (lexical) ddmax}. \Cref{tab:data-loss} shows this result of this experiment. For almost all input formats, \approach achieved a lower data loss than syntactic and lexical ddmaz (except for INI and SExp). This performance is attributed to the rich fasilure feedback of \approach, which allows it to distinguish between incomplete and incorrect input fragments.

\begin{result}
\approach has a high data recovery rate (88\%, on average) and its data %The data recovery and
