...
\title{Input Repair without Formal Specifications}
...
%\revise{
Large data repositories often have data that contains unintended errors.
Such errors may be introduced when
the data is being created (by humans or by buggy programs), modified (by external actors)
or transmitted (via flawed networks)~\cite{scaffidi2008accommodating}. 
For instance, several records are input by hand without validation,
leading to \textit{non-conforming inputs}~\cite{mucslu2015preventing}.
Such non-conforming records may also be caused by disagreements among data sources on the format specification which leads to different %variances in the
...
slightly different SQL formats~\cite{arvin2018comparison}, making their data dumps incompatible,
CSVs and other data formats often differ between applications of the same
company~\cite{taocp}, and  various
\<C> compilers provide slightly different interpretation of the \<C> language.
Even when a format is completely specified, such XML, there is no guarantee that
an input record can be processed by a given parser because the parser may be
non-conformant, implementing only a subset of the format~\cite{xmlconformance}.
These problems lead to non-conforming inputs that cannot be processed by their
conforming programs or consumed by end-users (e.g., developers). 
%subsequently, developers and software can not consume such inputs. 
%}

\revise{
Given such non-conforming inputs that are \emph{almost} but \emph{not quite}
parsable, developers are saddled with the task of \textit{input repair}. Input
repair is particularly important due to the high prevalence of non-conforming
inputs in software practice~\cite{kirschner2020debugging}. It can be challenging
to recover the conforming portion of non-conforming inputs
automatically~\cite{scaffidi2008topes}, and developers often have to
\emph{manually} repair such inputs~\cite{kirschner2020debugging}, which is
...
\revise{
%On one hand, g
Given a formal grammar for inputs, grammar-based input repair approaches such as 
