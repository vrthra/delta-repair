...
Our approach addresses these limitations of \ddmax, which we now discuss in detail.
...
%\revise{We discuss the limitations of \ddmax in detail.}
...
\item \textbf{Empirical Evaluation.} We evaluate \brepair using 806 (real-world) invalid inputs belonging to four well-known input formats (e.g., cJSON and TinyC). 
Our evaluation results show that %given input processors that can accurately
%signal \emph{incompleteness} or \emph{incorrectness} and % when given
%invalid inputs,
\textit{\brepair has a high (91\%) data recovery rate}. 
