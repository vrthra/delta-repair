...
\title{Repairing Inputs via Synthesis}
...
\revise{In particular, input repair is challenging without an input specification (e.g., input grammar) or program analysis}.
...
\revise{
Input data is prone to unintended errors. Such errors may be introduced when
...
\revise{
Given such invalid inputs that are \emph{almost} but \emph{not quite}
parsable, developers are saddled with the task of \textit{input repair}. Input
repair is particularly important due to the high prevalence of invalid inputs
in software practice~\cite{kirschner2020debugging}. It can be challenging
to recover the valid portion of invalid inputs
...
%
\ddmax_2(\cfail - \Delta_i, 2) & \text{else if $\exists i \in \{1, \dots, n\} \cdot \test(\cfail - \Delta_i) = \pass$ (``incr. to complem.'')} \\
%
\ddmax_2\bigl(\dpass \cup \Delta_i, \max(n - 1, 2)\bigr) &
\text{else if $\exists i \in \{1, \dots, n\} \cdot \test(\dpass \cup \Delta_i) = \pass $ (``incr. to subset'')} \\
%
...
\end{lstlisting}
This function does a \emph{binary search}
...
continuation (for a definition of \lstinline|sample_items_by_mask()|, see supplementary material).%\Cref{lst:sampling}).
...
error. Hence, we apply linear search instead (we provide code examples for binary and linear search in the supplementary material).% (\Cref{lst:extenditeminsert}).
\par
...
\noindent\textbf{Metrics and Measures:} We employ the following metrics and measures to evaluate repair quality: 

\noindent\textit{a.) Number of Repaired Inputs:} We count the number of files repaired before
...
\noindent\textit{d.) Runtime} is the time taken for input repair for each method.
...
is very effective in repairing invalid inputs, it repaired almost four out of every five invalid input withing a four minute timeout}, i.e., % \approach repaired 
...
\begin{result}
\approach is very effective in repairing invalid input files: %, and more effective than the state-of-the-art.
...
%Figure "Number of files repaired by each approach (bar chart) moved to supplementary material"
...
\noindent
\textbf{External Validity:} This refers to the \textit{generalizability} of our approach, i.e., 
%results, i.e., 
%We acknowledge 
...
\noindent
\textbf{Construct Validity:} This concerns the test oracle and failure feedback employed in our evaluation. To ensure all subjects provide the expected \textit{incomplete} and \textit{(in)correct} feedback, we tested the programs on sample invalid inputs and modified the subject program, if needed. 
...
\noindent
\textbf{Repair via Input Synthesis:} 
...
%\todo{we should provide a github page, including an anonymous notebook that allows to test the examples (in Table 1), it will be interesting}
 ...
 \begin{center}
% \vspace{-0.2mm}
     \textbf{\url{https://tinyurl.com/fsynth-artifact-icse2023}}%\todo{update link}
 \end{center}
