% TODO: YAML, TOML, NestedText, TSV, SQL data dumps, regexps like date and time, zip, address, phone numbers
%%
...
\newcommand\letterboxedDDRepair[1]{%
\textcolor{blue}{%
...
\newcommand\letterboxedinTableDDMax[1]{%
\textcolor{red}{%
\setlength{\fboxsep}{0pt}%
  \@tfor\@ii:=#1\do{%
    \fcolorbox{light-gray}{white}{\texttt{\strut\@ii}}%
  }%
}%
}


\newcommand\letterboxedinTableDDRepair[1]{%
\textcolor{blue}{%
\setlength{\fboxsep}{0pt}%
  \@tfor\@ii:=#1\do{%
    \fcolorbox{light-gray}{white}{\texttt{\strut\@ii}}%
...
\letterboxedinTableDDMax{\ \ \ \ 42\ }  &
\letterboxedinTableDDRepair{\{\ "name":\ "Dave"\ ,"age":\ 42\ \}} &
Limited repair \\
& & & options (deletion) \\
\hline
\letterboxedinTable{\{\ "item":\ "Apple",\ "price"} & \letterboxedinTableDDMax{\ \ \ \
3.45\ } & \letterboxedinTableDDRepair{\{\ "item":\ "Apple",\ "price"} & Rich structure
\\
\letterboxedinTable{:\ ***3.45\}} & &\letterboxedinTableDDRepair{:\ 3.45\}} &  (spans) \\
\hline
\letterboxedinTable{\{"ABCD":[*"1,2,3,4,5,6"]*\}} &
\letterboxedinTableDDMax{123456} &
\letterboxedinTableDDRepair{\{"ABCD":["1,2,3,4,5,6"]\}} &
Rich structure\\

& & &  (multiple-faults)  \\
\hline
\letterboxedinTable{2024/07/23T12:34:56Z} &
\textcolor{red}{\texttt{None}}  &
\letterboxedinTableDDRepair{2024-07-23T12:34:56Z} &
Require valid \\
& & & empty data\\
\hline
\end{tabular}}
...
specific formats, but variations due to factors such as human error or data 
...
data formats. We evaluate \drepair's performance against the state-of-the-art 
lexical and syntactic \ddmax techniques, using both synthetic and real-world
datasets.
...
modifying the parser to provide such feedback~\cite{mathis2019parser}.
...
\item \textbf{Empirical Evaluation.} % RESULTS
We evaluate \drepair using
...
Here, \\
\letterboxed{\{"ABCD":[*"1,2,3,4,5,6"]*\}} contains two distinct
corruptions.
...
removed. This results in the solution \letterboxedDDMax{123456} with
...
\letterboxed{*}. \ddmax repairs this input to \letterboxedDDMax{\ \ \ \ 3.45},
...
Here, there is a missing comma. The \ddmax repair of this string will
result in just
\textcolor{red}{%
\letterboxed{\ \ \ \ 42}%
}, which is an \emph{unexpected and significant reduction} from  the original string.
The issue is that, deletion of fragments alone can lead to significant
...
%\begin{flushleft}
%                                ^
\begin{lstlisting}[xleftmargin=0pt,escapeinside={(*@}{@*)}, basicstyle=\ttfamily\small,numbers=none]
$ echo -n '{"name": "Dave" "age":42}' | jq .
(*@\textcolor{red}{parse error: Expected separator at line 1, column 21}@*)
\end{lstlisting}
...
\begin{algorithmic}[1]
  ...
  \Function{BSearch}{array, parser, left=0}
...
\Function{DRepair}{string, parser}
    \State bndry $\gets$ \Call{BSearch}{string, parser}
    \State pq $\gets$ \Call{PriorityQueue}{}
    \State \Call{add}{pq, (string, bndry, $\emptyset$)}

    \While{pq $ \neq \emptyset $ }
        \State (string, bndry, fixes) $\gets$ \Call{top}{pq}
        \State deletes $\gets$ \{(bndry, $\emptyset$)\}
        \State inserts $\gets$ \{(bndry, c) $\,|\, c \in \alpha,$ 
        \State \,\, \Call{parser}{string[:bndry]+c} $\in$ \{ \Rincomplete, \Rvalid \}\}

        \For{fix \textbf{in} deletes $\cup$ inserts}
            \State new\_str $\gets$ \Call{ApplyFixes}{string, fixes + [fix]}
            \State new\_bndry $\gets$ \Call{BSearch}{new\_str, parser, bndry}

            \If{\Call{parser}{new\_str[:new\_bndry]} $=$ \Rvalid }
              \If{ string[new\_bndry:] $=\emptyset$ }
                \State \Return new\_str
              \EndIf
            \EndIf

            \State \Call{add}{pq, (string, new\_bndry, fixes + [fix])}
        \EndFor
    \EndWhile
...
\letterboxed{\{\ "name":\ "Dave"\ } and the remaining-suffix
\letterboxed{"age":\ 42\ \}}
...
\item Extending the parser-boundary by appending remaining characters in the
 remaining-suffix, we have\\
\letterboxedDDRepair{\{\ "name":\ "Dave"\ ,"age":\ 42\ \}},
...
resulting in
\letterboxedDDRepair{\{"ABCD":["1,2,3,4,5,6"]\}}.
...
\letterboxed{***3.45\ \}}.
 %*"1,2,3,4,5,6"]*\}}.
\item Since \letterboxed{*}
is not a valid character to add, \approach deletes this character. Deletion of
the two remaining characters will result in \letterboxedDDRepair{\{"item":\ "Apple",\ "price":\ 3.45\ \}},
...
\letterboxedDDRepair{\{"item":\ "Apple",\ "price":\ "***3.45\ \}"\}}.
\end{enumerate}
That is, there may be multiple repair candidates, and as \drepair operates
without human intervention, it is unable to distinguish
the input semantics, which may affect the optimal repair.
...
This leads to the string \letterboxedDDRepair{"abcd"}, which is suboptimal.
\end{enumerate}
...
leading to the string \letterboxedDDRepair{"[1,2,3,4]"}.
...
the beginning of the string, resulting in \\
\letterboxed{\{"abcd":[1,2,3]\}}
...
We used four data formats and their corresponding programs
(\Cref{tab:subjectprograms}).
These are INI (INI), JSON (cJSON), s-exprssions (SExp), and TinyC (TinyC).
Each program is moderately large (between 500 LOC to 3500 LOC), relatively
mature (7 to 14 years of development), and written in \<C>.

\subsection{Test Data} 

% 806 - SExp  (150) = 656 - 150 = 506 / 447
We evaluate our approach using 400 files, (100 each per format).
...
\begin{description}[wide]
% \item[\textbf{(2) ANTLR:}] We use ANTLR as the first baseline \formatdependent
...
The results are tabulated in \Cref{tab:effectiveness}.
...
Overall, \drepair repaired 70\% of corrupt data (353 out of 506). % VERIFY
...
\begin{tabular}{|p{3.0cm}|p{1cm}|p{1cm}|p{1cm}|p{1cm}|}
\hline
