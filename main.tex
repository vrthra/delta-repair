...
How effective is our approach (\approach) in repairing failure-inducing inputs? How effective is our approach, i.e., what is the time performance of \approach ?
...
%\revise{
How does \approach compare to the state-of-the-art input debugging techniques, in terms of effcectivness, efficiency, data recovery and diagnosis?
...
In this section, we discuss the findings %results 
of our empirical evaluations. 
% for each of the posed research questions. 

\noindent
\textbf{RQ1 Effectiveness:} 
We evaluate the effectiveness of our approach using \recheck{807} invalid input files from \recheck{four} inout formats (namely) .  
%and efficiency of our approach 
Specifically, we measure total number of repaired files for all four input formats. We also compare the effectiveness of \approach to that of \recheck{four} state-of-the-art techniques, namely the error-recovery of the subject program (\textit{baseline}) and \textit{ANTLR}, as well as the two maximizing variants of delta debugging, i.e., \textit{lexical ddmax} and \textit{syntactic ddmax}. 
...
\textit{our approach (\approach) is highly effective in repairing invalid input files, it repaired about four out of every five invalid input}. \approach repaired about \recheck{81\% (650 out of 807)} of invalid inputs, and also \textit{outperformed all four state-of-the art-techniques by up to \recheck{X\%}}. 
...
In summary, these results suggest that (a) \approach is highly effective in repairing invalid input files, (b) our approach more effective than the baselines, and (c) the combination of rich failure feedback and input synthesis outperforms is important for the repair of invalid input files. 
