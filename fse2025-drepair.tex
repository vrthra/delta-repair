...
\begin{algorithmic}[1]
  ...
  \Function{BoundarySearch}{array, parser, left=0, right=len(array)}
    \If{ \Call{parser}{array} $= \Rincomplete $}
        \State \Return right
    \EndIf
    \While{left < right - 1}
        \State middle $\gets$ (left + right) // 2
        \If{ \Call{parser}{array[:middle]} $= \Rincomplete $}
...
\Function{DRepair}{string, parser}
    ...
    \While{pq $ \neq \emptyset $ }
        ...
        \State dfix $\gets$ (boundary, string[boundary], 'delete')
        \State ifixes $\gets$ [(boundary, c, 'insert') for c in alphabet]

        \For{fix \textbf{in} [dfix] + ifixes}
            ...
            \If{\Call{parser}{new\_string, new\_boundary} == valid}
                \State \Return new\_string
            \EndIf

            \State \Call{add}{(pq, new\_string, new\_boundary, fixes + [fix])}
        \EndFor
    \EndWhile
\EndFunction
