...
The requirements of \drepair are similar to that of \ddmax. \drepair also requires
(1) A way to identify valid inputs.
However, instead of a (2) minimal valid starting sequence, and the (3) existence
of $\delta$s producing valid inputs, which may not always be available, \drepair requires
the following:
% As with \ddmax, we require the data processor to indicate if the given record
%is conforming to the expected format (i.e., valid).  
