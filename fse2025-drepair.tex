...
in reverse. It requires: (1) a method to identify valid inputs, (2) a starting 
minimal subsequence of the input that is \emph{valid}, and (3) the ability to 
add fragments ($\delta$s) to this input, resulting in larger \emph{valid 
inputs}. Given these prerequisites and a corrupt record that induces a parse 
error, \ddmax identifies $\delta$s that, when 
inserted into the minimal valid subsequence, avoid triggering the error.
This process generates increasingly larger valid inputs, and in doing so, it 
isolates and removes the part of the record causing the parse error, with 
minimal data loss.

While \ddmax has proven effective in certain scenarios, it has three major 
limitations: (1) \ddmax only allows fragment deletion ($\delta$s) as a repair 
operation, which is suboptimal because corrupt records often \emph{lack data} 
rather than contain extra data preventing parsing. (2) It cannot repair complex 
corruptions involving multiple faults. (3) It requires a \emph{valid empty 
record} to start from and \emph{valid records as waypoints} to the maximally
valid record. These limitations hinder \ddmax's ability to achieve maximal data 
repair in several cases, leading to significant data loss.
Table~\ref{tab} illustrates examples where these limitations result in severe
data loss, particularly when compared to \dtask using formal specifications.

The central idea of this paper is that many parsers return
\emph{parse-failure feedback} containing crucial information about why parsing
failed. This feedback can be leveraged to identify possible repairs that prevent
such failures. By using parse-failure feedback, our approach supports not only 
\emph{deletion} (as \ddmax does) but also \emph{insertion} and
\emph{substitution} as repair operations.

The requirements for \drepair are similar to those of \ddmax. Like \ddmax, 
\drepair requires (1) a way to identify valid inputs. However, instead of 
requiring a minimal valid starting sequence (requirement 2 of \ddmax) and the 
existence of $\delta$s producing valid inputs (requirement 3 of \ddmax, which 
may not always be available), \drepair requires the following: When given a 
nonconforming record, \drepair \textit{requires} the data processor to indicate 
whether the record is merely \emph{incomplete} (i.e., the record fragment is a 
prefix of a conforming record) or \emph{incorrect} (i.e., no suffix to this 
record fragment will result in a conforming record). 
...
We note that most parsers provide the requisite feedback.
In cases where such a feedback is not reliably obtained, it can be obtained
through external instrumentation as demonstrated by
Björn et al.~\cite{mathis2019parser}.
