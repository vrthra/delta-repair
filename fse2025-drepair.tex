...
\hline
  \textbf{Improvement} &  \textbf{2\%}  & $-$5\% & \textbf{10\%} & $-$19\% & \textbf{28\%} \\
\hline
\end{tabular}
...
a format-agnostic strategy, relying solely on parser feedback, making it a 
...
Our results show that \drepair achieves 17\% more perfect repairs,
delivers 1.7x higher-quality repairs, and recovers 23\% more valid data on
average against the next best solution.
...
flexible alternative to format-dependent approaches.
\end{abstract}
...
\emph{format-agnostic} (\Cref{tab:effectivenessnogrammar})
and \emph{format-aware} (\Cref{tab:effectivenessgrammar}) techniques.
...
\drepair, excluding empty files and whitespace, as shown in \Cref{tab:datarecovery}.
