...
\begin{filecontents*}{.effectivenessgrammar.tex}
\begin{table}[!tbp]
\centering
  \caption{Number of repaired corrupt records \textbf{data-format aware} repair approaches.
  The highest number of repaired inputs are in bold. The \textbf{Improvement} is the difference between \drepair and syntactic \ddmax.
  }
\begin{tabular}{|p{4.0cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|}
\hline
&  \multicolumn{5}{c|}{\textbf{Number of Repaired Inputs}}  \\
&  \multicolumn{1}{c|}{\textbf{ALL}} & \multicolumn{4}{c|}{\textbf{Data Format}}  \\
\textbf{Techniques} & \textbf{Total} \textbf{(\%)} & \textbf{INI} & \textbf{cJSON} & \textbf{SExp} & \textbf{TinyC} \\
\hline
\textbf{ANTLR} & 344 (43\%) & 133 & 69 & 96 &  46   \\
\textbf{Syntactic \ddmax} & 614 (76\%) & \textbf{201}  & 144  & \textbf{149}  & 120  \\ 	
\hline
\textbf{\approach}  & \textbf{624} (\textbf{77\%}) & 191 & \textbf{158}  & 121  &  \textbf{154} \\
\hline
\textbf{Improvement} &  2\%  & $-$5\% & \textbf{10\%} & $-$19\% & \textbf{28\%} \\
\hline
\end{tabular}
\label{tab:effectivenessgrammar}
\end{table}
\end{filecontents*}
...
\begin{table}[!tbp]
\centering
\caption{Number of repaired corrupt records for \textbf{format-agnostic} \dtask approaches.
  The highest number of repaired inputs are in bold. The \textbf{Improvement} is the difference between \drepair and lexical \ddmax.}
...
\textbf{Lexical \ddmax} & 578 (72\%) & \textbf{201}  & 132  & \textbf{131} & 114  \\ 		
\hline	
\textbf{\approach}  & \textbf{624} (\textbf{77\%}) & 191 & \textbf{158}  & 121  & \textbf{154} \\
\hline
\textbf{Improvement} &  \textbf{8\%}  & $-$5\% & \textbf{20\%} & $-$8\% & \textbf{35\%} \\
\hline
\end{tabular}
\label{tab:effectivenessnogrammar}
\end{table}
...
\hline
 &  \multicolumn{5}{c|}{\textbf{Average Repair Distance}}  \\
&  \multicolumn{1}{c|}{\textbf{ALL}} &   \multicolumn{4}{c|}{\textbf{Data Format}}  \\
\textbf{Techniques} & \textbf{Average}  & \textbf{INI} & \textbf{cJSON} & \textbf{SExp} & \textbf{TinyC} \\
\hline
...
\hline
\textbf{Impr. over Lex. \ddmax} & \textbf{1.7$\times$} & \textbf{1.1$\times$} & \textbf{2.1$\times$} & 0.8$\times$ & \textbf{4.7$\times$} \\
\textbf{Impr. over Syn. \ddmax} & \textbf{11.3$\times$} & \textbf{27.2$\times$} & \textbf{2.9$\times$} & \textbf{10.4$\times$} &\textbf{20.8$\times$} \\
\hline
\end{tabular}
% \hline
%  &  \multicolumn{5}{c|}{\textbf{\% Data Recovered by Approach}} &  \multicolumn{5}{c|}{\textbf{Average Data Loss }}  \\
% &  \multicolumn{1}{c|}{\textbf{ALL}} & \multicolumn{4}{c|}{\textbf{Data Format}}  &  \multicolumn{1}{c|}{\textbf{ALL}} & \multicolumn{4}{c|}{\textbf{Data Format}}  \\
...
\begin{table*}[!tbp]\centering
\caption{Data Recovery (file size difference) of \approach vs
...
\textbf{Techniques} & \textbf{Average} & \textbf{INI} & \textbf{cJSON} & \textbf{SExp} & \textbf{TinyC} \\
\hline
\textbf{Lexical \ddmax} & 81\%  & 85.8\% & 82.2\%   & 99.2\% & 47.7\% \\
\textbf{Syntactic \ddmax} & 74\%  &  70.6\% & 81.5\%  & 97.0\% & 47.0\% \\
\hline
\textbf{\approach} & \textbf{91\%}  &  \textbf{85.9\%} & \textbf{98.5\%} & \textbf{100\%} & \textbf{82.1\%} \\
\hline
\textbf{Impr. over Lex. \ddmax} & 12\% &  0.1\%  & \textbf{19.8\%} & 1.4\% & \textbf{72.1\%} \\
\textbf{Impr. over Syn. \ddmax} & \textbf{23\%}  &  \textbf{21.6\%}  & \textbf{20.8\%} & 3.7\% & \textbf{74.7\% }  \\
\hline
\end{tabular}
...
\Cref{sec:evaluation}, and provide results in \Cref{sec:results}
We further discuss the implications of this work in \Cref{sec:discussion},
...
We used four data formats and their corresponding programs (\Cref{tab:subjectprograms}). These are INI (INI), JSON (cJSON), S-Expressions (SExpParser) and TinyC (TinyC). Each program is moderately large (between 375 LOC to 3062 LOC), relatively mature (6 to 21 years old), and written in \<C>.
...
%of the number of real-world corrupt data and artificially synthesized corrupt data employed in our experiment.
...
methods is tabulated in \Cref{tab:effectivenessnogrammar}.
...
comparison to both \emph{format-agnostic} (\Cref{tab:effectivenessnogrammar})
and \emph{format-aware} (\Cref{tab:effectivenessgrammar}) \dtask techniques. 
It is also visualized in \Cref{fig:effectiveness}.
...
\noindent\textbf{RQ2: What is the average number of repair steps required for \dtask by \drepair?}
\Cref{tab:datarepairquality} shows the repair effectiveness achieved by each technique.
...
excluding empty files and white spaces. This is what we present in \Cref{tab:datarecovery}.
...
or both approaches. These are given in \Cref{fig:perfectlyrepairedvenndiagram}.
...
\approach is considerably efficient in input repair: It is reasonably fast, it takes about 10 seconds to repair an invalid input, on average.
However, it is about five times slower than \ddmax (two to three seconds, on average). 
\Cref{tab:efficiency}
shows that the execution time of \drepair
is much higher than that of syntactic and lexical \ddmax. 
This result is due to the number of program runs required by \approach, especially due to its additional repair operation (input synthesis) and its extra oracle checks (incompleteness and parse-boundary).
...
is 5$\times$ slower than \ddmax.
  %because it requires additional operations and oracle checks. 
\end{result}
...
%The \drepair also has several limitations. % validity threats:
...
%In summary, this work demonstrates that combining parse-failure feedback and input synthesis are important for the effective repair of invalid inputs, especially in the absence of an input specification 
