...
but in reverse.
Given a corrupt record which induces a parse error, and the following prerequisites:
(1) a way to decompose the record into independent tokens ($\delta$s),
(2) a way to identify valid inputs, and
(3) a starting minimal subsequence of the input that is valid, and the assurance
that if a $\delta$ does not contribute to the corruption, it can be merged to
the valid minimal input for a larger \emph{valid value}.
\ddmax thus successively identifies the $\delta$s that when inserted into the 
...
The requirements of \drepair are similar to that of \ddmax. \drepair needs
(1) a way to decompose the record into tokens, and
(2) a way to identify valid inputs.
(3) Instead of a minimal valid starting sequence, and the existence of
independent $\delta$s which may not always be available, \drepair requires
the following:
