...
a format-free strategy, relying solely on parser feedback, making it a 
...
\item[\textbf{(2) Lexical \ddmax:}] As the second baseline, we compare the
  \emph{format-free} lexical \ddmax described by Kirschner et al.~\cite{kirschner2020debugging}.
    This is the previous state-of-the-art when compared to \drepair (which is
    also format-free).
\end{description}


\noindent{\textbf{Format-dependent}.}
...
\begin{description}[wide]
\item[\textbf{(3) ANTLR:}] We use ANTLR as the first baseline format-dependent
...
%\centering
%  \caption{For \textbf{format-free} \dtask approaches.}
...
The repair effectiveness for format-free
and format-based
...
\emph{format-free}
and \emph{format-dependent}  techniques (\Cref{tab:effectiveness}).
%In this experiment, we measure the total number of files repaired by \drepair in
%comparison to both \emph{format-free} 
%and \emph{format-dependent}  \dtask techniques. 
...
We note that \drepair outperforms even the \emph{format-dependent} \dtask approaches
...
This paper presents \drepair, a novel format-free \dtask approach that
