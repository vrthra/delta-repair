...
(1) A way to identify valid inputs,
(2) A starting minimal subsequence of the input that is \emph{valid}, and 
(3) The ability to add fragments ($\delta$s) to this input resulting in larger
\emph{valid inputs}.
Given these prerequisites and a corrupt record that induces a parse error, 
\ddmax works as follows:
(1) It successively identifies $\delta$s that, when inserted into the minimal valid subsequence, avoid triggering the error.
(2) This process produces increasingly larger valid inputs.
(3) By doing so, it isolates and discards the parse-error inducing part of the record with minimal data loss.

%The result is a valid record with minimal data loss.
%\ddmax successively identifies the $\delta$s that when inserted into the 
%minimal valid subsequence, avoids triggering the error, producing larger valid inputs.
%This procedure isolates and discards the parse-error
%inducing part of the record resulting in a valid record with minimal data loss.
...
(1) \ddmax only allows skipping $\delta$s (fragment \emph{deletion}) as a repair operation, which
is non-optimal because corrupt records often \emph{lack data} rather than having extra data that prevent parsing.
(2) It cannot repair certain kinds of \emph{complex corruptions} containing multiple faults, and
(3) It requires the availability of a \emph{valid empty record} to start from,
and valid records as way-points to the maximally valid record.
...
The central idea in this paper is that many parsers will return a \emph{parse-failure
feedback} that contains crucial information about why the parse failed, and one
can leverage this feedback to identify the possible repairs that
can avoid such failure. Leveraging the parse-failure feedback allows our
approach to apply not just deletion (the only operation supported by \ddmax),
but also insertion and substitution as repair
operations.

The requirements of \drepair are similar to that of \ddmax. \drepair also has the
requirement (1) of \ddmax, that is, a way to identify valid inputs.
However, instead of the requirement (2) of \ddmax (i.e., a minimal valid
starting sequence), and the requirement (3), (i.e., the existence of $\delta$s
producing valid inputs, which may not always be available), \drepair requires
the following:
When given a nonconforming record, \drepair \textit{requires} the data processor
to indicate whether the record is merely \emph{incomplete} (that is, the record
fragment is a prefix of a conforming record)
or is \emph{incorrect} (that is, no suffix to this record fragment will result
in a conforming record). We note that this feedback is provided by most parsers.
In the cases where parsers do not provide this information, it can be obtained
by external instrumentation as demonstrated by Bj\"orn et al.~\cite{mathis2019parser}, or 
by modifying the parser to provide such failure feedback.

%\drepair leverages the parser response to predict which mutation is likely to fix the record with maximal data recovery. 
We note that similar to \ddmax, satisfying the requirements for \drepair does
not require using a formal grammar for parsing, and indeed, there are several
systems that implement handwritten parsers that satisfy \drepair constraints~\cite{eaton2021parser}.
Given parsers that conform to this requirement, \drepair can produce repairs
that result in maximal data-recovery, and minimal data-loss compared to \ddmax.
...
elicits the \emph{valid} response from the parser.

\item[repair-distance] The minimal number of repairs required to transform the corrupt
string to a complete string (also called \emph{edit-distance}). The larger the
